...
export function TimeSeriesClient({ initialMeters }: TimeSeriesClientProps) {
  const [meters, setMeters] = useState(initialMeters)
  const [isImporting, setIsImporting] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://ems-back.omarino.net'
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (ingestResponse.ok) {
        ...
        // Refresh meters list
        const refreshResponse = await fetch(`${API_URL}/api/meters`)
        if (refreshResponse.ok) {
          const updatedMeters = await refreshResponse.json()
